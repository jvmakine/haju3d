...
//        settings.setVSync(false);
    ...
    private static void make2dPerlinNoise(int w, int h) {
        Random random = new Random(0L);
        float[] data = new float[w * h];
        for (int sc = 1; sc != 128; sc *= 2) {
            add2dNoise(random, data, w, h, sc, FastMath.pow(0.5f * sc * 1.0f, 0.5f));
        }
        print2dPerlinNoise(data, w, h);
    }

    private static void print2dPerlinNoise(float[] data, int w, int h) {
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                if (data[x + y * w] > 0) {
                    System.out.print("#");
    ...
    private static void add2dNoise(Random random, float[] data, int w, int h, int scale, float amp) {
        int nw = w / scale + 2;
        int nh = h / scale + 2;
        int n = nw * nh;
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                float n1 = noise[x/scale + y/scale*nw];
                float n2 = noise[x/scale + 1 + y/scale*nw];
                float n3 = noise[x/scale + y/scale*nw + nw];
                float n4 = noise[x/scale + 1 + y/scale*nw + nw];
                
                float xt = (float)(x%scale)/scale;
                float yt = (float)(y%scale)/scale;
                
                data[x + y * w] += interpolateLinear2d(xt, yt, n1, n2, n3, n4);;
            }
        }
    }

    private static float[] make3dPerlinNoise(int w, int h, int d) {
        return make3dPerlinNoise(0L, w, h, d);
    }
...
//        rootNode.attachChild(SkyFactory.createSky(assetManager, "Textures/Sky/Bright/BrightSky.dds", false));
    ...
    private void setupRocks() {
        Texture rockTex = assetManager.loadTexture("Textures/Terrain/Rock2/rock.jpg");
        ...
        for (int i = 0; i < 200; i++) {
            ...
            if (collideWith != 0) {
                Vector3f pt = res.getClosestCollision().getContactPoint().subtract(0f, 0.1f, 0f);
                Geometry t = new Geometry("box", box);
                int col = rnd.nextInt(2);
                Material rock = makeColorMaterial(ColorRGBA.White);
                rock.setTexture("DiffuseMap", rockTex);
                t.setMaterial(rock);
                float angle = (float) (rnd.nextDouble() * Math.PI);
                t.setLocalRotation(new Quaternion(new float[] {0, angle, 0}));
                t.setLocalTranslation(pt);
...
//        characterObject = assetManager.loadModel("Models/Tree/Tree.mesh.j3o");
...
//           cArray[vi * 4 + 2] = 1.0f;
        ...
        //coloredMesh.setMaterial(makeColorMaterial(ColorRGBA.Green));
...
//        mat.setBoolean("UseVertexColor", false);
        ...
        //TODO add a texture!
//        Texture tex = assetManager.loadTexture("hvk/jmetesting/GrassGreenTexture0003b.jpg");
...
//        mat.setTexture("DiffuseMap", tex);
    ...
    private float getNoise(float[] noise, int tw, int th, int td, float tx, float ty, float tz) {
        ...
        if (x >= 0 && x < tw  -1 && y >= 0 && y < th - 1 && z >= 0 && z < td - 1) {
            //return noise[x + y * tw + z * tw * td];
            return interpolateLinear3d(xt, yt, zt,
    ...
    private void addQuad(
            ...
            Map<MyVertex, Integer> vertexIndex,
            MyVertex vector3f1,
            MyVertex vector3f2,
            MyVertex vector3f3,
            MyVertex vector3f4) {
...
//        }
//        System.out.println(mat.getMaterialDef().getMaterialParams());
...
//        dlsr.setShadowZFadeLength(300);
        viewPort.addProcessor(dlsr);
        
//        cam.setFrustumFar(50);
//        System.out.println("near: " + cam.getFrustumNear());
//        System.out.println("far:" + cam.getFrustumFar());
        
//        FilterPostProcessor fpp = new FilterPostProcessor(assetManager);
//        SSAOFilter ssaoFilter = new SSAOFilter(13f * 1.0f, 44f * 0.5f, 0.3f, 0.4f);
...
//        viewPort.addProcessor(fpp);
    ...
    private void initGrid() {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                for (int z = 0; z < d; z++) {
                    boolean inside = FastMath.sqr(w/2 - x) + FastMath.sqr(h/2 - y) + FastMath.sqr(d/2 - z) < FastMath.sqr(w/2);
                    grid.set(x, y, z, inside ? Tile.GROUND : Tile.AIR);
                }
            }
        }
    }
    ...
    private void initGrid2() {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                for (int z = 0; z < d; z++) {
                    //float v = FastMath.sqr(w/2 - x) + FastMath.sqr(h/2 - y) + FastMath.sqr(d/2 - z);
