...
public class BenchmarkMeshGenerationApp {
  public static void main(String[] args) {
    ...
    for (int x = 0; x < 3; x++) {
      for (int y = 0; y < 3; y++) {
        for (int z = 0; z < 3; z++) {
          ...
          Chunk chunk = new Chunk(chunkSize, 0, chunkPosition);
          world.setChunk(chunkPosition, chunk);
        }
      }
    }
    ...
    for (int i = 0; i < 10; i++) {
      ...
      MyMesh cubeMesh = ChunkSpatialBuilder.makeCubeMesh(world, new ChunkPosition(1, 1, 1), new ChunkLightManager());
